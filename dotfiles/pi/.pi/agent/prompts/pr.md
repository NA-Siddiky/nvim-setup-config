---
name: pr
description: Generate a high-signal PR description from branch diffs and open a PR via gh CLI.
argument-hint: "[target-branch]"
---

Role: Tech Lead. Output ONLY the exact markdown. No fluff.

Target branch: The PR should merge into `$1` (if no argument was given, default to `main`).

Rules:

1. Analyze the diff between the target branch (`$1` or `main` if not provided) and the current branch.
...
7. Use gh cli to create a pr targeting the resolved target branch (`$1` if provided, otherwise `main`). Use the correct username, email, and repo name.
